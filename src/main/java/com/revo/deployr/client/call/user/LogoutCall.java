...
/**
 * Provides support for DeployR API call: /r/user/logout.
 * <p/>
 * Simply construct an instance of this call and pass it on the
 * execute() method of your {@link com.revo.deployr.client.RClient}.
 */
public class LogoutCall extends AbstractCall implements Callable<RCoreResult> {

    private final String API = REndpoints.RUSERLOGOUT;

    public LogoutCall(RUserDetails details) {
        if (details != null) {
            httpParams.put("usercookie", details.cookie);
        }
