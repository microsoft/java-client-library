...
/**
 * Provides support for DeployR API call: /r/repository/file/grant.
 * <p/>
 * Simply construct an instance of this call and pass it on the
 * execute() method of your {@link com.revo.deployr.client.RClient}.
 */
public class RepositoryFileGrantCall extends AbstractCall
        implements Callable<RCoreResult> {

    private final String API = REndpoints.RREPOSITORYFILEGRANT;

    public RepositoryFileGrantCall(RRepositoryFileDetails repoFile,
                                   String newauthor,
                                   String revokeauthor) {
        httpParams.put("filename", repoFile.filename);
        httpParams.put("directory", repoFile.directory);
        httpParams.put("newauthor", newauthor);
        httpParams.put("newauthor", revokeauthor);
