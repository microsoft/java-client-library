...
/**
 * Provides support for DeployR API call: /r/project/recycle.
 * <p/>
 * Simply construct an instance of this call and pass it on the
 * execute() method of your {@link com.revo.deployr.client.RClient}.
 */
public class ProjectRecycleCall extends AbstractCall
        implements Callable<RCoreResult> {

    private final String API = REndpoints.RPROJECTRECYCLE;

    public ProjectRecycleCall(String project,
                              boolean preserveWorkspace,
                              boolean preserveDirectory) {
        httpParams.put("project", project);
        httpParams.put("preserveworkspace", Boolean.toString(preserveWorkspace));
        httpParams.put("preservedirectory", Boolean.toString(preserveDirectory));
